...
int main(int argc, char *argv[]) {
    ...
    mqd_t s1_queue, rsp_queue;
    Message req_msg;
    Message rsp_msg;

    // Open the S1 queue for reading
    ...
    printf("Service 1 Worker: Started processing jobs...\n");

    while (true) {
        ssize_t bytes_read = mq_receive(s1_queue, (char *)&req_msg, sizeof(req_msg), NULL);
        if (bytes_read == -1) {
            perror("mq_receive");
            break;
        }

        printf("Service 1 Worker: Received job ID %d with data %d\n", req_msg.RequestID, req_msg.data);
        ...
        printf("Service 1 Worker: Completed job ID %d with result %d\n", rsp_msg.ResponseID, rsp_msg.result);
    }

    mq_close(s1_queue);
    mq_close(rsp_queue);
    printf("Service 1 Worker: Exiting...\n");
    return 0;
}
